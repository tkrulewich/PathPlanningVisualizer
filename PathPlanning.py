...
class Grid:
    ...
    def draw(self) -> None:
        ...
        x_list = [node.x for node in self.nodes.values() if not node.visited]
        y_list = [node.y for node in self.nodes.values() if not node.visited]
        ...
        x_list2 = [node.x for node in self.nodes.values() if node not in self.valid_nodes]
        y_list2 = [node.y for node in self.nodes.values() if node not in self.valid_nodes]
